...
/**
 * MED-AI: PERSONALIZED EDITION (V7.3 - Clean UI & Final Polish)
 * - Removed: All text from loading screens (Animation only).
 * - Fixed: Browser History (Undo/Redo works).
 * - Fixed: Layout Alignment (Perfectly centered).
 */
...
if (!firebaseConfig.apiKey) {
  ...
  } catch (e) { console.warn("Env vars missing."); }
}
...
const HistoryModal = ({ item, onClose }) => {
  ...
  const prec = item.precautions || "No detailed precautions archived.";
  ...
  return (
    <div className="otp-overlay" onClick={onClose}>
      <div className="glass-card modal-card anim-scale-in" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-section"><div className="modal-label">DATE & TIME</div><div className="modal-value">{item.date} at {item.time}</div></div>
        <div className="modal-section"><div className="modal-label">TOP 3 DISEASE INDICATIONS</div><div className="predictions-list">{predictions.slice(0,3).map((p, i) => (<div key={i} className="prediction-item"><span className="rank-badge">#{i+1}</span><strong className="disease-name">{p}</strong></div>))}</div></div>
        <div className="modal-section"><div className="modal-label">SYMPTOMS REPORTED</div><div className="tags-container">{syms.map((s, i) => (<span key={i} className="symptom-tag-static">{s.replace(/_/g, ' ')}</span>))}</div></div>
        <div className="advice-box"><div className="modal-label" style={{color: '#F59E0B'}}>MEDICAL PRECAUTIONS & ADVICE</div><p>{prec}</p></div>
...
export default function App() {
  ...
  const [userProfile, setUserProfile] = useState({
    ...
    email: "", phone: "", city: "", state: "", country: "", photo: "", password: "" 
  });
    ...
    try {
      ...
      if (profileSnap.exists()) {
        ...
        const recoveryProfile = {
          name: email ? email.split('@')[0] : "User", email: email || "",
          age: "", weight: "", height: "", gender: "", phone: "", city: "", state: "", country: "", photo: ""
        };
        try {
  ...
  const handleProfileChange = (e) => setUserProfile(prev => ({ ...prev, [e.target.name]: e.target.value }));
  const handleLoginChange = (e) => setLoginData(prev => ({ ...prev, [e.target.name]: e.target.value }));
  const handlePasswordChangeInput = (e) => setPasswordChange(prev => ({ ...prev, [e.target.name]: e.target.value }));
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    setError(null);
    try { await signInWithEmailAndPassword(auth, loginData.email, loginData.password); } catch (err) { setError("Invalid credentials."); }
  };
  ...
  const handleRegister = async (e) => {
    ...
    try {
        ...
        await setDoc(doc(db, 'artifacts', appId, 'users', userCredential.user.uid, 'profile', 'main'), { ...safeProfile, email: userProfile.email });
        await setDoc(doc(db, 'artifacts', appId, 'users', userCredential.user.uid, 'data', 'history'), { records: [] });
        setIsNewUser(false);
        setNotification("Account created! Log in.");
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      if (currentUser.email !== userProfile.email) await updateEmail(currentUser, userProfile.email);
  ...
  async function analyzeSymptoms() {
    ...
    // Set screen directly to "loading" (without pushing state) 
    // so "Undo/Back" from results goes to symptoms, not stuck on loading.
    setScreen("loading"); 
    ...
    try {
      ...
      setPrecautions(data.precaution);
      
      // Push "Results" state so we can go back
      navigate("results");
    } catch (err) {
      setError("Connection failed.");
      setScreen("symptoms"); // Go back to symptoms on error
    }
  }
  ...
  if (authLoading) {
    return (
      <div className="med-ai-root">
        <style>{css}</style>
        <div className="loading-state" style={{height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center'}}>
           <div className="neural-spinner-lg"><div className="spinner-ring ring-1"></div><div className="spinner-core"><Icons.Brain size={40}/></div></div>
  ...
  return (
    <div className="med-ai-root">
      ...
      {viewingHistoryItem && <HistoryModal item={viewingHistoryItem} onClose={() => setViewingHistoryItem(null)} />}

      <nav className="top-nav">
        <div className="nav-container glass-card">
          <div className="brand" onClick={() => navigate(isLoggedIn ? "home" : "login")}>
          ...
          {isLoggedIn && (
            <div className="profile-nav-section">
              <div className={`user-indicator ${showProfileMenu ? 'active' : ''}`} onClick={() => setShowProfileMenu(!showProfileMenu)}>
                {userProfile.photo ? <img src={userProfile.photo} alt="Profile" className="nav-profile-img" /> : <div className="nav-profile-placeholder"><Icons.User /></div>}
              </div>
              {showProfileMenu && (
                <div className="profile-dropdown glass-card anim-fade-in-down">
                  <div className="dropdown-header">
                    <div className="header-photo">{userProfile.photo ? <img src={userProfile.photo} alt="Profile" /> : <div className="placeholder-icon"><Icons.User /></div>}</div>
                    <div className="header-info"><span className="name">{userProfile.name}</span><span className="email">{userProfile.email}</span></div>
                  </div>
                  <div className="dropdown-divider"></div>
                  <button className="menu-btn" onClick={() => { navigate("profile"); setShowProfileMenu(false); }}><Icons.Edit /> Manage Profile</button>
                  <div className="dropdown-divider"></div>
      ...
      <main className="main-stage">
        {screen === "login" && (
          <div className="login-section glass-card anim-scale-in">
            <div className="form-header"><div className="logo-lg"><Icons.Brain size={64}/></div><h2>{isNewUser ? "Join Med-AI" : "Clinical Access"}</h2><p>{isNewUser ? "Register to start." : "Enter credentials."}</p></div>
            {error && <div className="error-bar">{error}</div>}
            {!isNewUser ? (
              <form className="profile-form" onSubmit={handleLogin}>
                <div className="form-row full"><label>Email</label><input required name="email" value={loginData.email} onChange={handleLoginChange} /></div>
                <div className="form-row full"><label>Password</label><input required type="password" name="password" value={loginData.password} onChange={handleLoginChange} /></div>
                <button type="submit" className="primary-button full-width glow-effect">Access Dashboard</button>
                <div className="form-footer"><span className="footer-text">New here?</span><button type="button" className="text-link" onClick={() => setIsNewUser(true)}>Create Profile</button></div>
              </form>
            ) : (
              <form className="profile-form" onSubmit={handleRegister}>
                <div className="photo-upload-container"><label className="photo-label"><div className="photo-preview-box">{userProfile.photo ? <img src={userProfile.photo} alt="Preview" className="photo-preview" /> : <div className="photo-placeholder"><Icons.Camera /><span>Upload</span></div>}</div><input type="file" accept="image/*" onChange={handlePhotoUpload} className="hidden-input" /></label></div>
                <div className="form-row full"><label>Full Name</label><input required name="name" value={userProfile.name} onChange={handleProfileChange} /></div>
                <div className="form-group-2"><div className="form-row"><label>Gender</label><div className="select-wrapper"><select name="gender" value={userProfile.gender} onChange={handleProfileChange} className="select-input"><option value="">Select</option><option value="Male">Male</option><option value="Female">Female</option></select></div></div><div className="form-row"><label>Email</label><input required type="email" name="email" value={userProfile.email} onChange={handleProfileChange} /></div></div>
                <div className="form-row full"><label>Set Password</label><input required type="password" name="password" value={userProfile.password} onChange={handleProfileChange} /></div>
        ...
        {screen === "profile" && (
          <div className="anim-fade-in">
            <div className="section-header"><h2>My Profile</h2><p>Manage identity.</p></div>
            {error && <div className="error-bar">{error}</div>}
            <div className="profile-grid-layout">
              <div className="glass-card padding-lg">
                <h3>Personal Details</h3>
                <form className="profile-form" onSubmit={handleUpdateProfile}>
                  <div className="photo-upload-container"><label className="photo-label"><div className="photo-preview-box">{userProfile.photo ? <img src={userProfile.photo} alt="Preview" className="photo-preview" /> : <div className="photo-placeholder"><Icons.Camera /><span>Update</span></div>}</div><input type="file" accept="image/*" onChange={handlePhotoUpload} className="hidden-input" /></label></div>
                  <div className="form-row full"><label>Name</label><input name="name" value={userProfile.name} onChange={handleProfileChange} /></div>
                  <div className="form-group-3"><div className="form-row"><label>Age</label><input type="number" name="age" value={userProfile.age} onChange={handleProfileChange} /></div><div className="form-row"><label>Weight</label><input type="number" name="weight" value={userProfile.weight} onChange={handleProfileChange} /></div><div className="form-row"><label>Height</label><input type="number" name="height" value={userProfile.height} onChange={handleProfileChange} /></div></div>
                  <div className="form-group-2"><div className="form-row"><label>City</label><input name="city" value={userProfile.city} onChange={handleProfileChange} /></div><div className="form-row"><label>Country</label><input name="country" value={userProfile.country} onChange={handleProfileChange} /></div></div>
              ...
              <div className="glass-card padding-lg h-fit">
                ...
                <form className="profile-form" onSubmit={handleChangePassword}>
                  <div className="form-row full"><label>Current Password</label><input type="password" name="current" value={passwordChange.current} onChange={handlePasswordChangeInput} /></div>
                  <div className="form-row full"><label>New Password</label><input type="password" name="new" value={passwordChange.new} onChange={handlePasswordChangeInput} /></div>
                  <button type="submit" className="btn-secondary full-width" style={{marginTop: "10px"}}>Change Password</button>
                </form>
                <div style={{marginTop: "30px", textAlign: 'center'}}><button type="button" className="text-link" onClick={() => navigate("home")}>&larr; Back to Dashboard</button></div>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="screen-home anim-fade-in">
            <div className="hero-grid">
              <div className="hero-content">
                ...
                <p>Sophisticated medical pattern identifying engine. Connected as <strong>{userProfile.name}</strong>.</p>
                {bmiValue && <div className={`bmi-mini-card glass-card ${bmiInfo.class}`}><div className="icon-circle"><Icons.Scale /></div><div className="bmi-text"><span className="label">Clinical BMI</span><span className="value">{bmiValue} <span className="status">({bmiInfo.label})</span></span></div></div>}
                <div className="hero-actions"><button className="primary-button lg glow-effect" onClick={startNewAssessment}>Start New Assessment</button></div>
              </div>
              <div className="history-sidebar glass-card">
                <div className="sidebar-header"><Icons.History /><span>Recent History</span></div>
                <div className="history-list">{history.length > 0 ? history.map((item, idx) => (<div key={idx} className="history-item" onClick={() => setViewingHistoryItem(item)}><div className="history-content"><div className="history-date">{item.date} • {item.time}</div><div className="history-result">{item.topMatch}</div></div><div className="arrow-icon">&rarr;</div></div>)) : <div className="history-empty">No clinical history recorded yet.</div>}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {screen === "symptoms" && (
          <div className="symptoms-section anim-fade-in">
            <div className="header-row">
              <div className="section-title"><h2>Select Biomarkers</h2><span className="user-welcome">Analyzing patterns for <strong>{userProfile.name}</strong></span></div>
              <div className="search-wrapper glass-card"><Icons.Search /><input placeholder="Search symptoms..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
            </div>
            <div className="biomarker-grid">{filteredSymptoms.map(s => (<div key={s} className={`marker-card glass-card ${selectedSymptoms[s] ? 'selected' : ''}`} onClick={() => toggleSymptom(s)}><div className="check-circle">{selectedSymptoms[s] && <Icons.Check />}</div><span>{s.replace(/_/g, " ")}</span></div>))}</div>
            <div className="floating-dock glass-card">
              <div className="dock-stats"><span className="count">{Object.values(selectedSymptoms).filter(Boolean).length}</span><span className="label">Biomarkers</span></div>
              <div className="dock-btns"><button className="btn-secondary" onClick={() => navigate("home")}>Cancel</button><button className="btn-action glow-effect" onClick={analyzeSymptoms}>Analyze Patterns</button></div>
            </div>
          </div>
        )}
        ...
        {screen === "loading" && (
          <div className="loading-state anim-fade-in">
            <div className="neural-spinner-lg"><div className="spinner-ring ring-1"></div><div className="spinner-ring ring-2"></div><div className="spinner-core"><Icons.Brain size={40}/></div></div>
            {/* Text removed as requested */}
          </div>
        )}
        ...
        {screen === "results" && (
          <div className="results-section anim-fade-in">
            <div className="results-intro"><div className="mini-badge success">SCREENING COMPLETE</div><div className="results-title-row"><h2>Analysis Summary</h2><button className="export-btn glass-card" onClick={exportReport}><Icons.Download /> Export Report</button></div></div>
            <div className="results-grid">
              <div className="primary-result glass-card glow-border"><div className="result-clinical-meta"><span className="tier-tag">PRIMARY INDICATION</span><span className={`bmi-badge ${bmiInfo.class}`}>BMI: {bmiValue} ({bmiInfo.label})</span></div><h3>{results[0]}</h3><p>Pattern correlation suggests high alignment with clinical signatures.</p><div className="confidence-bar"><div className="fill" style={{width: '88%'}}></div></div></div>
              <div className="side-results">{results.slice(1, 3).map((r, i) => (<div key={i} className="mini-result glass-card"><span className="tier-tag">ALTERNATIVE {i + 1}</span><h4>{r}</h4></div>))}</div>
            </div>
            <div className="precautions-section glass-card"><div className="section-icon"><Icons.Lock /></div><div className="content"><h3>Guidance Protocols</h3><p>{precautions}</p></div></div>
            <div className="medical-disclaimer"><div className="disclaimer-header">⚠️ MEDICAL SAFETY PROTOCOL</div><p>Not a medical diagnosis. Consult a physician.</p><button className="outline-btn" onClick={() => navigate("home")}>Return to Dashboard</button></div>
          </div>
        )}
      </main>
    </div>
